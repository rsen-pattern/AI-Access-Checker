...
def render_results(audit: dict, get_secret_fn) -> None:
    ...
    for test_url, js_r in js_results.items():
        ...
        with st.expander(f"{label} — Score: {js_r['score']}/100" + (f" (via {provider})" if provider else "")):
            ...
            if comp:
                ...
                # AI Analysis (memoized + suppressed when crawl is unreliable)
                if _audit_unreliable:
                    ai_analysis = None
                else:
                    ai_analysis = audit.get("_bifrost_js", {}).get(test_url)
                    if not ai_analysis:
                        try:
    ...
    # AI Analysis — What This Means (memoized + suppressed when crawl is unreliable)
    if _audit_unreliable:
        robots_ai = None
    else:
        robots_ai = audit.get("_bifrost_robots")
        if not robots_ai:
            try:
    ...
    for test_url, sr in schema_results.items():
        ...
        with st.expander(f"{label} — {len(schemas)} schema item(s), Score: {sr.get('score', 0)}/100 ({grade_letter})"):
            ...
            if _audit_unreliable:
                ...
                schema_ai = audit.get("_bifrost_schema", {}).get(test_url)
                if not schema_ai:
                    try:
    ...
    for test_url, sem_r in semantic_results.items():
        ...
        with st.expander(f"{label}"):
            ...
            if _audit_unreliable:
                ...
                sem_ai = audit.get("_bifrost_sem", {}).get(test_url)
                if not sem_ai:
                    try:
    ...
    if bifrost_key:
        ...
        if _audit_unreliable:
            ...
            with st.spinner("Generating Pattern Brain analysis..."):
                ...
                brain_analysis = audit.get("pattern_brain")
